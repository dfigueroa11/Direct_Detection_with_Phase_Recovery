...
def per_layer_loss_onehotness1(x_oh, a, w, device):
    loss_l = torch.zeros(x_oh.size(0), 1, device=device)        # Denotes the loss in Layer L
    for l, x_oh_l in enumerate(x_oh):
...
def per_layer_loss_onehotness2(x_oh, a, w, device):
    loss_l = torch.zeros(x_oh.size(0), 1, device=device)        # Denotes the loss in Layer L
    for l, x_oh_l in enumerate(x_oh):
        loss_l[l] = torch.log(torch.Tensor([l+2]).to(device))*torch.mean(torch.mean(a*16*torch.square((torch.square(x_oh_l)-x_oh_l)),1))
    return loss_l
