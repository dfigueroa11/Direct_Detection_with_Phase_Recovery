...
class PhaseDetNet(nn.Module):
    ...
    def forward(self, l, x_phase, x_phase_oh, v, x_mag, y_e, y_o, Psi_e, Psi_o):
        ...
        q = x_phase[-1] - self.delta1_l[l]*torch.bmm(A_e,y_e.unsqueeze(-1)).squeeze(-1) + self.delta2_l[l]*torch.bmm(A_e,Psi_e_x_sql).squeeze(-1) \
        ...
        x_phase_oh = x_phase_oh + self.linear_trafo_2_l[l](z)
        # proyect and append result
        x_phase = torch.cat((x_phase, aux_func.oh_2_sym(self.mapp, x_phase_oh, self.sym_len, self.device).unsqueeze(0)), 0)
