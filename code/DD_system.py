...
class DD_system():
    ...
    def square_law_detection(self,signal):
        ...
        shot_noise = abs_signal * torch.normal(0., self.sigma_sh, size=signal.size(), dtype=torch.float64, device=self.device)
        thermal_noise = torch.normal(0., self.sigma_th, size=signal.size(), dtype=torch.float64, device=self.device)
