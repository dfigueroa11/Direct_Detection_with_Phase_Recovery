...
sym_mem_aux_ch_list = [*range(7,8)]

#################### Simulation #########################

ser = torch.empty((len(sym_mem_aux_ch_list),len(SNR_dB_list)), dtype=torch.float)

for i,sym_mem_aux_ch in enumerate(sym_mem_aux_ch_list):
    ...
    for j,SNR_dB in enumerate(SNR_dB_list):
        ...
        ser[i,j] = ch_met.get_ER(info_symbols, symbols_hat)
