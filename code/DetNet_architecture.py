...
class DetNet(nn.Module):
    ...
    def __init__(self, layers, block_len, sym_mem , one_hot_len, v_len, z_len, device):
        ...
        for i in range(layers):
            nn.init.normal_(self.linear_trafo_1_l[i].weight, std = 0.4)
            nn.init.normal_(self.linear_trafo_1_l[i].bias, std = 0.4)
        ...
        for i in range(0, layers):
            nn.init.normal_(self.linear_trafo_2_l[i].weight, std = 0.4)
            nn.init.normal_(self.linear_trafo_2_l[i].bias, std = 0.4)
        ...
        for i in range(0, layers):
            nn.init.normal_(self.linear_trafo_3_l[i].weight, std = 0.4)
            nn.init.normal_(self.linear_trafo_3_l[i].bias, std = 0.4)
